...
# Reconstruct the user's app with a robust project-root import fix.
...
# ---------------------------------------------------------------------------
# IMPORTANT FIX FOR "No module named 'database'" / LINE 9 IMPORT ERRORS
# ---------------------------------------------------------------------------
# Streamlit Cloud sometimes starts the app from a different working directory.
# Put the repository root on sys.path before importing local project modules.
...
try:
    ...
    if exc.name == "database":
        st.error(
            "Database module not found. Make sure your GitHub repository contains "
            "`database/db.py` and `database/__init__.py`."
        )
        st.stop()
    raise
...
.cockpit-sub {
  font-size:11px; color:#64748b; letter-spacing:0.14em;
...
.score-mid {
  color:#fde047; background:rgba(250,204,21,0.10);
...
@media (max-width: 900px) {
  .cockpit-right { flex-wrap:wrap; justify-content:flex-end; }
  .mrow, .kpi-row { grid-template-columns:repeat(2,1fr); }
}
</style>
""",
...
def _get_api_key():
    """Return the API key from Streamlit secrets, environment, or UI input."""
    try:
...
def _safe_text(value):
    """Prevent accidental HTML injection in values rendered with unsafe HTML."""
    import html
...
with tabs[0]:
    st.markdown(
        f"""
<div class="qhead">
  <div class="qhead-title">
    Question {q_now} of {total_q} - {_safe_text(st.session_state.itype)}
  </div>
        ...
        with c2:
            if st.button(
                ...
                if not api_key:
                    ...
                    and not st.session_state.resume_text
                    and not st.session_state.jd_text
                ):
                    st.warning(
                        "Upload Resume and/or JD first, then click Analyze."
                    )
                else:
...
<div class="panel">
  <div class="ai-row">
    ...
    <div>
      ...
      <div class="ai-role">
        {_safe_text(st.session_state.personality)} - {_safe_text(diff)} tier
      </div>
    </div>
  </div>
...
""",
            ...
            weak = (
                st.session_state.manager.weak_areas[-3:]
                if st.session_state.manager
                else []
            )
            ...
            if weak:
                chips = "".join(
                    f'<span class="tag">{_safe_text(w)}</span>'
                    for w in weak
                )
                st.markdown(
                    '<div class="panel">'
...
<div class="panel">
  ...
  <div class="panel-sub">
    ...
    the tech stack, and one measurable outcome. Avoid buzzwords.
  </div>
</div>
""",
                unsafe_allow_html=True,
...
""",
                ...
                try:
                    ...
                    st.info(
                        "Audio recording is not available in this "
                        "Streamlit version. Upload an audio file instead."
                    )
                ...
                up = st.file_uploader(
                    ...
                    label_visibility="collapsed",
                    key="answer_audio_upload",
                )

                if up is not None:
                    audio_data = up

                if audio_data is not None and st.button(
                    ...
                    with st.spinner("Transcribing..."):
                        ...
                        try:
                            ...
                            st.session_state.speech_metrics = analyze_speech(
                                text,
                                30,
                            )
                ...
                answer_text = st.text_area(
                    ...
                    height=260,
                    key="typed_answer",
                    placeholder="Type your answer - be specific and concise...",
                )
...
<div class="mrow">
  <div class="mtile">
    <div class="v">{m.get("wpm", 0)}</div>
    <div class="l">Speaking Pace</div>
  </div>
  <div class="mtile">
    <div class="v">{m.get("filler_total", 0)}</div>
    <div class="l">Filler Words</div>
  </div>
  <div class="mtile">
    <div class="v">{m.get("duration_sec", 0)}s</div>
    <div class="l">Duration</div>
  </div>
  <div class="mtile ok">
    <div class="v">{m.get("clarity", 0)}/10</div>
    <div class="l">Clarity Score</div>
  </div>
</div>
""",
                unsafe_allow_html=True,
                ...
                if not answer_text and not skip:
                    ...
                    if not grounded.get("grounded", True):
                        ev["hallucination_risk"] = {
                            ...
                            "unsupported_claims": grounded.get(
                                "unsupported_claims",
                                [],
                            ),
                        }
            ...
            if finish and st.session_state.history:
                ...
                st.success(
                    f"Session #{st.session_state.session_id} saved - "
                    f"avg {avg:.1f}/10"
                )
...
""",
                ...
                if ev.get("strengths"):
                    ...
                    st.markdown(
                        "".join(
                            f'<span class="chip-ok">{_safe_text(s)}</span>'
                ...
                if ev.get("gaps"):
                    ...
                    st.markdown(
                        "".join(
                            f'<span class="chip-gap">{_safe_text(s)}</span>'
                            for s in ev["gaps"]
                        ),
                        unsafe_allow_html=True,
                    )
...
""",
            ...
            with c1:
                st.markdown(
                    f"""
<div class="panel">
  <div class="panel-title">Your Answer</div>
  <div class="panel-sub">{_safe_text(last["answer"])}</div>
</div>
""",
                    unsafe_allow_html=True,
                )
                ...
                st.markdown(
                    f"""
<div class="panel">
  <div class="panel-title">Ideal Model Answer</div>
  <div class="panel-sub">
    {_safe_text(ev.get("ideal_answer") or "-")}
  </div>
</div>
""",
                    unsafe_allow_html=True,
            ...
            with c2:
                ...
                st.markdown(
                    f"""
<div class="panel">
  <div class="panel-title">What Was Missed</div>
  <div class="panel-sub">{missed_html}</div>
</div>
""",
                    unsafe_allow_html=True,
                )
            ...
            with e1:
                st.download_button(
                    ...
                    md,
                    file_name=(
                        f"report_{datetime.utcnow():%Y%m%d_%H%M}.md"
                    ),
                    mime="text/markdown",
                    use_container_width=True,
                )
            ...
            with e2:
                st.download_button(
                    ...
                    html,
                    file_name=(
                        f"report_{datetime.utcnow():%Y%m%d_%H%M}.html"
                    ),
                    mime="text/html",
                    use_container_width=True,
                )
        ...
        for i, h in enumerate(hist, 1):
            ...
            with st.expander(
                ...
                if ev.get("ideal_answer"):
                    st.markdown(
                        f"**Ideal:** {ev['ideal_answer']}"
                    )

                if ev.get("suggested_best_answer"):
                    st.markdown(
                        f"**Suggested:** "
                        f"{ev['suggested_best_answer']}"
                    )
...
with tabs[2]:
    ...
    if (
        ...
        st.info(
            "Upload Resume + JD in the sidebar, then click "
            "Analyze Documents."
        )
    else:
        if st.button(
        ...
        if rep:
            ...
            items = [
                (
                    k1,
                    "Match",
                    f'{rep.get("match_percent", 0)}%',
                ),
                (
                    k2,
                    "Skills",
                    rep.get("technical_skills", {}).get(
                        "score",
                        0,
                    ),
                ),
            ...
            for col, label, val in items:
                with col:
                    st.markdown(
                        f"""
<div class="mtile">
  <div class="v">{_safe_text(val)}</div>
  <div class="l">{_safe_text(label)}</div>
</div>
""",
                        unsafe_allow_html=True,
                    )
...
<div class="panel">
  ...
  <div class="panel-sub">
    <b>Before:</b> {_safe_text(r.get("before", ""))}
  </div>
  <div class="panel-sub" style="margin-top:6px;">
    <b style="color:#6ee7b7;">After:</b>
    {_safe_text(r.get("after", ""))}
  </div>
</div>
""",
                    unsafe_allow_html=True,
...
<div class="panel">
  <div class="panel-title">
    Problem - {_safe_text(prob["difficulty"])}
  </div>
  <div class="qtext">{_safe_text(prob["prompt"])}</div>
  <div class="panel-sub" style="margin-top:8px;">
    <b>Examples:</b> {_safe_text(prob["examples"])}
  </div>
</div>
""",
        unsafe_allow_html=True,
...
""",
        ...
        st.markdown(
            f"**Time:** {res.get('time_complexity', '-')}"
        )
        st.markdown(
            f"**Space:** {res.get('space_complexity', '-')}"
        )
        ...
        if res.get("edge_cases"):
            st.markdown(
                ...
                + ", ".join(
                    str(x)
                    for x in res["edge_cases"]
                )
            )

        if res.get("follow_up"):
            st.info(
                f"**Follow-up:** {res['follow_up']}"
            )
...
with tabs[4]:
    ...
    grounded_pct = (
        int((grounded_ok / total_q_now) * 100)
        if total_q_now
        else 0
    )

    trend_html = (
        '<span class="trend">no baseline</span>'
    )
    ...
    if len(sessions) >= 2 and sessions[0]["avg_score"]:
        ...
        if prev > 0:
            ...
            if delta > 0.3:
                trend_html = (
                    f'<span class="trend" '
                    f'style="color:#6ee7b7;">'
                    f'+{delta:.1f} vs last</span>'
                )
            elif delta < -0.3:
                trend_html = (
                    f'<span class="trend" '
                    f'style="color:#fca5a5;">'
                    f'{delta:.1f} vs last</span>'
                )
...
<div class="empty">
  ...
  <div class="p">Start an interview in Tab 1.</div>
</div>
""",
            unsafe_allow_html=True,
        )
...
""",
        ...
        with c1:
            summary = (
                _summary_scores(hist)
                if hist
                else {d: 0 for d in DIMENSIONS}
            )
        ...
        with c2:
            if hist:
                ...
                st.info(
                    "Complete a question to see the trajectory."
                )
            ...
            for col, icon, label, val, color in stat_items:
                with col:
                    st.markdown(
                        f"""
<div class="kpi">
  <div class="icon">{_safe_text(icon)}</div>
  <div class="val" style="color:{color};">
    {_safe_text(val)}
  </div>
  <div class="lbl">{_safe_text(label)}</div>
</div>
""",
                        unsafe_allow_html=True,
                    )
            ...
            if len(sessions) > 1:
                st.markdown(
                    '<div class="section-title">Session Progression</div>',
                    unsafe_allow_html=True,
                )
        ...
        if hist:
            st.markdown(
                '<div class="section-title">Preparation Matrix</div>',
                unsafe_allow_html=True,
            )

            weak = (
                st.session_state.manager.weak_areas
                if st.session_state.manager
                else []
            )

            strong = (
                st.session_state.manager.strong_areas
                if st.session_state.manager
                else []
            )
        ...
        if st.session_state.speech_metrics:
            st.markdown(
                '<div class="section-title">Speech Analytics</div>',
                unsafe_allow_html=True,
            )
...
<div class="kpi">
  <div class="icon">{_safe_text(icon)}</div>
  <div class="val">{_safe_text(val)}</div>
  <div class="lbl">{_safe_text(label)}</div>
</div>
""",
                        unsafe_allow_html=True,
...
<div class="section-title">
  Session History
  <span class="count">{len(sessions)} SAVED</span>
</div>
""",
        unsafe_allow_html=True,
    )
...
<div class="empty">
  ...
  <div class="p">Finish an interview in Tab 1.</div>
</div>
""",
            unsafe_allow_html=True,
        )
        ...
        if sort_by == "Newest first":
            ...
            data.sort(
                key=lambda x: x.get("avg_score") or 0,
                reverse=True,
            )
        ...
        if confirm_wipe:
            st.warning(
                "Wipe-all is enabled. This cannot be undone."
            )
        ...
        for s in data:
            ...
            col_info, col_score, col_view, col_del = (
                st.columns([6, 1.2, 1, 1])
            )
...
<div class="session-row">
  <div class="sid">#{_safe_text(s["id"])}</div>
  <div class="meta">
    <div class="role">
      {_safe_text(s.get("role", "-"))}
    </div>
    <div class="sub">
      {_safe_text(s.get("interview_type", "-"))}
      -
      {_safe_text(s.get("personality", "-"))}
      -
      {_safe_text(s.get("difficulty", "-"))}
      -
      {_safe_text(created)}
    </div>
  </div>
...
""",
            ...
            with col_view:
                if st.button(
                    ...
                    st.session_state["_view_session"] = s[
                        "id"
                    ]
        ...
        if view_id:
            st.markdown(
                f"""
<div class="section-title">
  Session #{_safe_text(view_id)} Details
</div>
""",
                unsafe_allow_html=True,
            )
