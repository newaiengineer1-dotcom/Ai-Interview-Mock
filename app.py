...
from database.db import init_db, create_session, add_turn, update_session, get_sessions, get_turns, delete_session, delete_all_sessions, get_session_stats
...
init_db()
st.set_page_config(page_title="InterviewAI Studio", page_icon="AI", layout="wide", initial_sidebar_state="expanded")
...
with tabs[0]:
    ...
    if st.session_state.manager and st.session_state.current_question:
        ...
        with left:
            ...
            if weak:
                chips = "".join(f'<span class="tag">{w}</span>' for w in weak)
            ...
            <div class="panel">
              ...
              <div class="panel-sub">Answer in 30-60 seconds. Reference a specific project, the tech stack, and one measurable outcome. Avoid buzzwords.</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with right:
            ...
            if mode == "Voice":
                ...
                try:
                    ...
                    pass
                up = st.file_uploader("or upload audio", type=["wav", "mp3", "m4a", "webm"], label_visibility="collapsed")
                if up is not None:
                    audio_data = up
            ...
            if submit or skip:
                if not answer_text and not skip:
                    ...
                    with st.spinner("Evaluating..."):
                        ev = evaluate_answer(q, ans, st.session_state.role, r_ctx, j_ctx, api_key=st.session_state.api_key)
                    with st.spinner("Groundedness check..."):
                        grounded = check_groundedness(q, ans, r_ctx + j_ctx, api_key=st.session_state.api_key)
                    ...
                    with st.spinner("Next question..."):
                        next_q = generate_question(st.session_state.role, st.session_state.itype, st.session_state.personality, st.session_state.manager.difficulty, r_ctx, j_ctx, st.session_state.history, api_key=st.session_state.api_key)
        ...
        if st.session_state.history:
            ...
            with f2:
                ...
                <div class="panel">
                  ...
                  <div style="font-size:2rem;font-weight:800;color:#67e8f9;">{ev.get("overall",0):.1f}<span style="font-size:1rem;color:#94a3b8;">/10</span></div>
                  <div class="panel-sub" style="margin-top:6px;">{ev.get("verdict","")}</div>
                </div>
                """, unsafe_allow_html=True)
                if ev.get("strengths"):
                    st.markdown("**Strengths**")
                    st.markdown("".join(f'<span class="chip-ok">{s}</span>' for s in ev["strengths"]), unsafe_allow_html=True)
                if ev.get("gaps"):
                    st.markdown("**Gaps**")
                    st.markdown("".join(f'<span class="chip-gap">{s}</span>' for s in ev["gaps"]), unsafe_allow_html=True)
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.markdown(f'<div class="panel"><div class="panel-title">Your Answer</div><div class="panel-sub">{last["answer"]}</div></div>', unsafe_allow_html=True)
                st.markdown(f'<div class="panel"><div class="panel-title">Ideal Model Answer</div><div class="panel-sub">{ev.get("ideal_answer") or "-"}</div></div>', unsafe_allow_html=True)
            with c2:
                missed = ev.get("missed", [])
                missed_html = "<br>".join("- " + x for x in missed) if missed else "-"
    ...
    if not st.session_state.resume_text or not st.session_state.jd_text:
        ...
        if st.button("Run ATS Analysis", use_container_width=True):
            with st.spinner("Analyzing..."):
                st.session_state.ats_report = analyze_resume_jd(st.session_state.resume_text, st.session_state.jd_text, api_key=st.session_state.api_key)
        rep = st.session_state.ats_report
        if rep:
            k1, k2, k3, k4 = st.columns(4)
            for col, label, val in [(k1, "Match", f'{rep.get("match_percent",0)}%'), (k2, "Skills", rep.get("technical_skills",{}).get("score",0)), (k3, "Projects", rep.get("projects",{}).get("score",0)), (k4, "Impact", rep.get("impact",{}).get("score",0))]:
                with col:
                    st.markdown(f'<div class="mtile"><div class="v">{val}</div><div class="l">{label}</div></div>', unsafe_allow_html=True)
            ...
            with c1:
                ...
                st.markdown("".join(f'<span class="chip-ok">{s}</span>' for s in rep.get("strong_matches", [])), unsafe_allow_html=True)
            with c2:
                st.markdown("**Missing Gaps**")
                st.markdown("".join(f'<span class="chip-gap">{s}</span>' for s in rep.get("missing_gaps", [])), unsafe_allow_html=True)
            for r in rep.get("rewrites", []):
                st.markdown(f'<div class="panel"><div class="panel-title">Rewrite</div><div class="panel-sub"><b>Before:</b> {r.get("before","")}</div><div class="panel-sub" style="margin-top:6px;"><b style="color:#6ee7b7;">After:</b> {r.get("after","")}</div></div>', unsafe_allow_html=True)
...
with tabs[3]:
    ...
    st.markdown(f'<div class="panel"><div class="panel-title">Problem - {prob["difficulty"]}</div><div class="qtext">{prob["prompt"]}</div><div class="panel-sub" style="margin-top:8px;"><b>Examples:</b> {prob["examples"]}</div></div>', unsafe_allow_html=True)
    ...
    if st.button("Evaluate Code", use_container_width=True):
        if not code.strip():
            ...
            with st.spinner("Analyzing..."):
                st.session_state.coding_result = evaluate_code(prob["title"], prob["prompt"], code, lang, api_key=st.session_state.api_key)
    res = st.session_state.coding_result
    if res:
        c1, c2, c3 = st.columns(3)
        for col, label, val in [(c1, "Correctness", res.get("correctness",{}).get("score",0)), (c2, "Code Quality", res.get("code_quality",{}).get("score",0)), (c3, "Overall", res.get("overall",0))]:
            with col:
                st.markdown(f'<div class="mtile"><div class="v">{val}</div><div class="l">{label}</div></div>', unsafe_allow_html=True)
...
with tabs[4]:
    ...
    if not hist and not sessions:
        ...
        if stats["total"] > 0:
            ...
            for col, icon, label, val, color in [(s1, "S", "Total Sessions", stats["total"], "#67e8f9"), (s2, "A", "Avg Score", f'{stats["avg_score"]:.1f}', "#a5b4fc"), (s3, "B", "Best Session", f'{stats["best_score"]:.1f}', "#6ee7b7"), (s4, "L", "Lowest Session", f'{stats["worst_score"]:.1f}', "#fca5a5")]:
                with col:
                    st.markdown(f'<div class="kpi"><div class="icon">{icon}</div><div class="val" style="color:{color};">{val}</div><div class="lbl">{label}</div></div>', unsafe_allow_html=True)
            if len(sessions) > 1:
        ...
        if st.session_state.speech_metrics:
            ...
            for col, icon, label, val in [(sa, "W", "WPM", m["wpm"]), (sb, "F", "Fillers", m["filler_total"]), (sc, "N", "Words", m["words"]), (sd, "C", "Clarity", f'{m["clarity"]}/10')]:
                with col:
                    st.markdown(f'<div class="kpi"><div class="icon">{icon}</div><div class="val">{val}</div><div class="lbl">{label}</div></div>', unsafe_allow_html=True)
...
with tabs[5]:
    ...
    if not sessions:
        ...
        for s in data:
            ...
            with col_info:
                st.markdown(f'<div class="session-row"><div class="sid">#{s["id"]}</div><div class="meta"><div class="role">{s.get("role","-")}</div><div class="sub">{s.get("interview_type","-")} - {s.get("personality","-")} - {s.get("difficulty","-")} - {created}</div></div></div>', unsafe_allow_html=True)
            with col_score:
        ...
        if view_id:
            st.markdown(f'<div class="section-title">Session #{view_id} Details</div>', unsafe_allow_html=True)
