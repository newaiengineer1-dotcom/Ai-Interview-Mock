...
from services.groq_service import chat, transcribe, get_client
...
from agents.evaluator import evaluate_answer, evaluate_code, DIMENSIONS
...
with st.sidebar:
    ...
    st.session_state.role = st.text_input("Target Role", value=st.session_state.role or "Software Engineer")
    st.session_state.itype = st.selectbox("Interview Type", INTERVIEW_TYPES,
                                          index=INTERVIEW_TYPES.index(st.session_state.itype))
    st.session_state.personality = st.selectbox("Interviewer Personality",
                                                list(PERSONALITIES.keys()),
                                                index=list(PERSONALITIES.keys()).index(st.session_state.personality))
    st.session_state.difficulty = st.selectbox("Starting Difficulty", DIFFICULTY_LEVELS,
    ...
    if st.button("🔄 Reset Session", use_container_width=True):
        for k in ["manager", "session_id", "history", "current_question",
                  "analyzed", "ats_report", "speech_metrics", "coding_result"]:
            st.session_state[k] = None if k in ("manager", "current_question", "ats_report",
                                                 "speech_metrics", "coding_result") else ([] if k == "history" else False)
...
# ---------- Tabs ----------
tabs = st.tabs(["🎯 Interview", "🧠 Resume & ATS", "💻 Coding", "📊 Dashboard", "📁 History"])

# ======================= TAB 1: INTERVIEW =======================
with tabs[0]:
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.markdown(f'<div class="metric"><div class="v">{len(st.session_state.history)}</div><div class="l">Questions</div></div>', unsafe_allow_html=True)
    with col2:
        scores = [h["evaluation"].get("overall", 0) for h in st.session_state.history]
        avg = sum(scores)/len(scores) if scores else 0
        st.markdown(f'<div class="metric"><div class="v">{avg:.1f}</div><div class="l">Avg Score</div></div>', unsafe_allow_html=True)
    with col3:
        diff = st.session_state.manager.difficulty if st.session_state.manager else st.session_state.difficulty
        st.markdown(f'<div class="metric"><div class="v">{diff}</div><div class="l">Difficulty</div></div>', unsafe_allow_html=True)
    with col4:
        sid = st.session_state.session_id or "—"
        st.markdown(f'<div class="metric"><div class="v">#{sid}</div><div class="l">Session</div></div>', unsafe_allow_html=True)
    ...
    if st.session_state.manager is None:
        if st.button("🚀 Start Interview", use_container_width=True):
            if not st.session_state.analyzed and not st.session_state.resume_text:
                ...
                r_ctx = retriever.retrieve(st.session_state.role + " " + st.session_state.itype,
                                           k=4) if retriever else []
                j_ctx = []
                if retriever:
                    j_ctx = [d for d in retriever.docs if d["category"].startswith("jd_")][:3]
    ...
    if st.session_state.manager and st.session_state.current_question:
        ...
        <div class="card" style="margin-bottom:12px;">
          <div style="font-size:0.75rem;color:#94a3b8;text-transform:uppercase;letter-spacing:0.1em;">
            Question {len(st.session_state.history)+1} • {st.session_state.manager.difficulty}
          </div>
          <div style="font-size:1.15rem;color:#f9fafb;margin-top:8px;font-weight:500;">
            {st.session_state.current_question}
          </div>
        </div>""", unsafe_allow_html=True)

        mode = st.radio("Answer mode", ["✍️ Type", "🎙️ Voice"], horizontal=True, key="answer_mode")

        answer_text = ""
        speech = None

        if mode == "✍️ Type":
            answer_text = st.text_area("Your answer", height=160, key="typed_answer",
                                       placeholder="Type your answer — be specific, concise, and reference your own experience…")
        else:
            ...
            if audio_data is not None and st.button("📝 Transcribe"):
                with st.spinner("Transcribing via Whisper…"):
                    ...
                    try:
                        ...
                        speech = analyze_speech(text, 30)  # approximate
                        st.session_state.speech_metrics = speech
                    except Exception as e:
                        st.error(f"Transcription failed: {e}")

            answer_text = st.text_area("Transcript (edit if needed)", 
        ...
        if submit or skip:
            if not answer_text and not skip:
                ...
                st.session_state.manager.record(st.session_state.current_question, ev)
                st.session_state.history.append({
                    "question": st.session_state.current_question,
                    "answer": ans,
                    "evaluation": ev,
                })
                add_turn(st.session_state.session_id, len(st.session_state.history),
                         st.session_state.current_question, ans,
                         ev, ev.get("scores", {}))
                ...
                with st.spinner("Thinking of next question…"):
                    next_q = generate_question(
                        st.session_state.role, st.session_state.itype,
                        st.session_state.personality, st.session_state.manager.difficulty,
    ...
    if st.session_state.history:
        ...
        with c2:
            st.markdown(f'<div class="card"><b style="color:#a5b4fc;">Overall: {ev.get("overall", 0):.1f} / 10</b><br>'
                        f'<i style="color:#94a3b8;">{ev.get("verdict","")}</i></div>',
                        unsafe_allow_html=True)
            if ev.get("strengths"):
                st.markdown("**✅ Strengths**")
                st.markdown("".join(f'<span class="chip-ok">{s}</span>' for s in ev["strengths"]),
                            unsafe_allow_html=True)
            if ev.get("gaps"):
                st.markdown("**⚠️ Gaps**")
                st.markdown("".join(f'<span class="chip-gap">{g}</span>' for g in ev["gaps"]),
                            unsafe_allow_html=True)

        st.markdown("#### 🧭 Comparative Review")
        c1, c2 = st.columns(2)
        with c1:
            st.markdown('<div class="qa-card"><b>YOUR ANSWER</b><br>' + last["answer"] + '</div>',
                        unsafe_allow_html=True)
            st.markdown('<div class="qa-card ideal"><b>IDEAL MODEL ANSWER</b><br>' +
                        (ev.get("ideal_answer") or "—") + '</div>', unsafe_allow_html=True)
        with c2:
        ...
        with c1:
            st.download_button("⬇️ Download Markdown", md,
                               file_name=f"interview_report_{datetime.utcnow().strftime('%Y%m%d_%H%M')}.md",
                               mime="text/markdown", use_container_width=True)
        with c2:
            st.download_button("⬇️ Download HTML", html,
                               file_name=f"interview_report_{datetime.utcnow().strftime('%Y%m%d_%H%M')}.html",
                               mime="text/html", use_container_width=True)

        # Finalize session on demand
...
with tabs[1]:
    ...
    if not st.session_state.resume_text or not st.session_state.jd_text:
        st.info("Upload both a Resume and a Job Description in the sidebar, then click **Analyze Documents**.")
    else:
        ...
        if rep:
            ...
            with c1:
                st.markdown(f'<div class="metric"><div class="v">{rep.get("match_percent",0)}%</div><div class="l">Match</div></div>', unsafe_allow_html=True)
            for label, key, col in [("Skills","technical_skills",c2),
                                    ("Projects","projects",c3),
                                    ("Impact","impact",c4)]:
                with col:
                    v = rep.get(key, {}).get("score", 0)
                    st.markdown(f'<div class="metric"><div class="v">{v}</div><div class="l">{label}</div></div>', unsafe_allow_html=True)
            ...
            with c1:
                ...
                st.markdown("".join(f'<span class="chip-ok">{s}</span>' for s in rep.get("strong_matches", [])),
                            unsafe_allow_html=True)
            with c2:
                st.markdown("#### ⚠️ Missing Gaps")
                st.markdown("".join(f'<span class="chip-gap">{g}</span>' for g in rep.get("missing_gaps", [])),
                            unsafe_allow_html=True)

            st.markdown("#### ✍️ Bullet Rewrites (XYZ / STAR)")
            for r in rep.get("rewrites", []):
                st.markdown(f'<div class="qa-card missed"><b>Before:</b> {r.get("before","")}<br><b>After:</b> {r.get("after","")}</div>',
                            unsafe_allow_html=True)
            ...
            for k in ["technical_skills", "projects", "impact", "ats_readiness", "role_alignment"]:
                v = rep.get(k, {})
                if v:
                    st.markdown(f"**{k.replace('_',' ').title()}** — {v.get('score','-')}/10: {v.get('notes','')}")
...
with tabs[2]:
    ...
    st.markdown(f'<div class="card"><b>{prob["title"]}</b> <span class="pill">{prob["difficulty"]}</span><br>'
                f'{prob["prompt"]}<br><i style="color:#94a3b8;">{prob["examples"]}</i></div>',
                unsafe_allow_html=True)
    lang = st.selectbox("Language", ["python", "javascript", "java", "cpp", "go"])
    code = st.text_area("Your solution", height=280, placeholder="# write your solution here")
    ...
    if res:
        ...
        with c1:
            st.markdown(f'<div class="metric"><div class="v">{res.get("correctness",{}).get("score",0)}</div><div class="l">Correctness</div></div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div class="metric"><div class="v">{res.get("code_quality",{}).get("score",0)}</div><div class="l">Code Quality</div></div>', unsafe_allow_html=True)
        with c3:
            st.markdown(f'<div class="metric"><div class="v">{res.get("overall",0)}</div><div class="l">Overall</div></div>', unsafe_allow_html=True)
...
with tabs[3]:
    ...
    if not hist:
        ...
        with c2:
            ...
            fig.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                              font_color="#e5e7eb", yaxis_range=[0, 10], height=350)
            st.plotly_chart(fig, use_container_width=True)
        ...
        with c1:
            ...
            for w in weak or ["—"]:
                st.markdown(f'<span class="chip-gap">{w}</span>', unsafe_allow_html=True)
        with c2:
            st.markdown("**✓ Strong / Interview Ready**")
            for s in strong or ["—"]:
                st.markdown(f'<span class="chip-ok">{s}</span>', unsafe_allow_html=True)

        if st.session_state.speech_metrics:
            st.markdown("#### 🎙️ Speech Analytics (last voice answer)")
            m = st.session_state.speech_metrics
            c1, c2, c3, c4 = st.columns(4)
            with c1: st.markdown(f'<div class="metric"><div class="v">{m["wpm"]}</div><div class="l">WPM</div></div>', unsafe_allow_html=True)
            with c2: st.markdown(f'<div class="metric"><div class="v">{m["filler_total"]}</div><div class="l">Fillers</div></div>', unsafe_allow_html=True)
            with c3: st.markdown(f'<div class="metric"><div class="v">{m["words"]}</div><div class="l">Words</div></div>', unsafe_allow_html=True)
            with c4: st.markdown(f'<div class="metric"><div class="v">{m["clarity"]}/10</div><div class="l">Clarity</div></div>', unsafe_allow_html=True)
...
with tabs[4]:
    ...
    if not sessions:
        ...
        df = pd.DataFrame(sessions)[["id", "created_at", "role", "interview_type", "personality", "difficulty", "avg_score"]]
        ...
        if len(sessions) > 1:
            ...
            fig = px.line(df2, x="id", y="avg_score", markers=True,
                          title="Performance Progression (avg score per session)")
            fig.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                              font_color="#e5e7eb", yaxis_range=[0,10])
            st.plotly_chart(fig, use_container_width=True)

        sid = st.selectbox("View session details", [s["id"] for s in sessions])
        turns = get_turns(sid)
        for i, t in enumerate(turns, 1):
            ev = t.get("evaluation", {})
            with st.expander(f"Q{i}: {t['question'][:90]} — {float(ev.get('overall',0)):.1f}/10"):
